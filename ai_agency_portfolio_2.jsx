...
const Portfolio = () => {
  ...
  const sqlDemos = [
    { id: 'analytics', title: 'Realtime Analytics', description: 'Live SQL Queries', query: 'SELECT date, SUM(revenue) FROM sales GROUP BY date', defaultMetrics: { queries: '1.2M/day', latency: '<50ms', uptime: '99.9%' } },
    { id: 'segmentation', title: 'Segmentation', description: 'Customer Segments', query: 'SELECT segment, COUNT(*) FROM customers GROUP BY segment', defaultMetrics: { segments: '12', accuracy: '94%', processing: '<2s' } },
    { id: 'reporting', title: 'Reporting', description: 'Automated Reports', query: 'SELECT category, metrics FROM reports', defaultMetrics: { reports: '500/mo', automation: '100%', saved: '200h' } }
  ];
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${darkMode ? 'dark bg-gray-950' : 'bg-gray-50'}`}>
        ...
        {mobileMenuOpen && (
          <div className="md:hidden border-t border-gray-200 dark:border-gray-800">
            <div className="px-4 py-4 space-y-3">
              ...
              <button onClick={() => setDarkMode(!darkMode)} className="flex items-center space-x-2 text-gray-700 dark:text-gray-300">
                {darkMode ? <Sun className="w-5 h-5" /> : <Moon className="w-5 h-5" />}
      ...
      {/* Hier kannst du den Rest deiner Sections einfügen */}
    </div>
  );
};
